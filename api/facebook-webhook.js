...
async function yorumaPublicAICevapUret(yorumMetni, dmGitti) {
  try {
    ...
    if (!response.ok) {
      const errBody = await response.text().catch(() => "");
      console.error("FB WEBHOOK: public yorum AI cevap hatasi:", response.status, errBody.slice(0, 300));
      return null;
    }
    const data = await response.json();
    return data.content?.[0]?.text || null;
  } catch (e) {
    console.error("FB WEBHOOK: public yorum AI cevap istisnasi:", e && e.message ? e.message : e);
    return null;
  }
}
