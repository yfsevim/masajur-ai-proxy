...
async function queryShipment(orderNumber, circuitBreaker, logPrefix) {
  ...
  try {
    ...
    // 2026-09-02 IKINCI DUZELTME (#12415 vakasi): yukaridaki isim-eslesmesi
    // TEK BASINA YETERSIZ cikti - Yurtici bazi iadelerde Alici Adi'ni musteri
    // olarak birakip "Iade Durumu"nu BASKA bir alanda tutuyor (#12415: Alici
    // Adi "FATIH TATLI" ama Yurtici panelinde ayrica "Iade Durumu: IADE" ve
    // "Teslim Alan: YAKUP SEVIM" gorunuyor - hangi XML etiketi oldugu henuz
    // kesinlesmedi, bkz. api/teslim-kontrol.js?mod=debug-kargo). Bu etiket
    // netlesene kadar GUVENLI TARAFTA KALMAK icin: ham XML cevabinin
    // TAMAMINDA "iade" gecen HER durumu da supheli sayiyoruz. Boyle bir
    // siparise otomatik fatura KESILMEZ, bunun yerine manuel kontrole
    // dusurulur - eksik fatura, yanlis kesilmis (ve iptali gereken) faturadan
    // her zaman daha az zararli.
    const xmlIcindeIadeGeciyor = /iade/i.test(xml);
    const sirketeIadeEdildi = operationStatus === "DLV" && (adAdresEslesmesi || xmlIcindeIadeGeciyor);
    ...
    if (sirketeIadeEdildi) {
      ...
      console.log(logPrefix + ": DLV ama supheli - PAKET MUSTERIYE GERCEKTEN TESLIM EDILMEMIS OLABILIR (" + sebep + ")");
    }
