...
async function queryShipment(orderNumber, circuitBreaker, logPrefix) {
  ...
  try {
    ...
    const sirketeIadeEdildi = operationStatus === "DLV" &&
      !!receiverCustName && KENDI_SIRKET_ADI_DESENI.test(receiverCustName);

    if (sirketeIadeEdildi) {
      console.log(logPrefix + ": DLV ama alici kendi sirketimiz - PAKET BIZE IADE EDILMIS, gercek teslimat degil:", receiverCustName);
    }
